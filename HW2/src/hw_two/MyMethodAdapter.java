...
public class MyMethodAdapter extends MethodNode {
	...
	/*
	 * Collects Defs and Uses for labels.
	 */
	@Override
	public void visitVarInsn(final int opcode, final int var) {
		if(var != 0) {
			if(opcode >= 21 && opcode <= 53) { // LOADS
				if(CFGBuilder.LABEL_TO_USE.get(lastLabelNode) == null) {
					HashSet<Integer> temp = new HashSet<Integer>();
					CFGBuilder.LABEL_TO_USE.put(lastLabelNode, temp);
				}
				CFGBuilder.LABEL_TO_USE.get(lastLabelNode).add(var);
			}else if(opcode >= 54 && opcode <= 86) { // STORES
				if(CFGBuilder.LABEL_TO_DEF.get(lastLabelNode) == null) {
					HashSet<Integer> temp = new HashSet<Integer>();
					CFGBuilder.LABEL_TO_DEF.put(lastLabelNode, temp);
				}
				CFGBuilder.LABEL_TO_DEF.get(lastLabelNode).add(var);
			}
		}
